...
#| Implement "rotate" to accept an xy-coordinate c
 ...
 | These rotations should be in the order 0°, 90° counter-clockwise, 180°, 270° counter-clockwise.
 | As a refresher, the 90° counter-clockwise rotation of
...
(define (rotate c)
  (list
  (cons (car c) (cdr c))
  (list (- 0 (car (reverse c))) (car c))
  (list (car c) (- 0 (car (reverse c))))
  (list (car (reverse c)) (car c))
  ))
...
#| "get-streetname" returns the street-name field.
...
(define (get-streetname b)
  car (car (reverse (car(car (cdr (cdr (cdr (cdr b))))))))
  )
