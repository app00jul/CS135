...
(define (p-prime? p)
  (cond [(pz? p) #f]
        [(p= p (succ z)) #f]
        [else (prime-inc p (succ(succ z)))]
   )
  )

(define (prime-inc pn inc)
  (cond [(p= pn inc) #t]
        [else (cond [(p-divides? inc pn) #f]
                    [else (prime-inc pn (succ inc))]
               )]
   )
  )
