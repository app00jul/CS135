...
#| Implement "sum-power" to accept positive integers p and n
 ...
 |   (sum-power 2 15) -> 1240
 |   (sum-power 5 8)  -> 61776
 |   (sum-power 10 7) -> 353815700
 |#

;; Type signature: (sum-power positive-int positive-int) -> int
;; 3 PTS
(define (sum-power p n)
  (cond [(<= n 0) 0]
        [else (+ (expt n p) (sum-power p (- n 1)))]
  )
  )
...
(define (subfact n)
  (cond [(eq? n 1) 0]
        [(eq? n 2) 1]
        [(eq? 1 (modulo n 2)) (- (* n (subfact(- n 1))) 1)]
        [else (+ 1 (* n (subfact(- n 1))))]
  )
  )
