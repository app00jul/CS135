...
(define (get-vertices G)
  (usort (cond [(equal? G '()) '()]
               [else (append (car G) (get-vertices (cdr G)))]
          )
   )
  )
...
;; 3 PTS
(define (all-even? G)
  (iter G (get-vertices G)
   )
  )

(define (iter G num_list)
  (cond [(equal? num_list '()) #t]
        [(even? (degree G (car num_list))) (iter G (cdr num_list))]
        [else #f]
   )
  )
...
(define (connected? G)
  (cond [(equal? G '()) #t]
        [(equal? (usort (dfs G (caar G))) (get-vertices G)) #t]
        [else #f]
   )
  )
...
;; 2 PTS
(define (eulerian? G)
  (cond [(and (all-even? G) (connected? G)) #t]
        [else #f]
   )
  )
