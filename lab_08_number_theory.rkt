...
;; 4 PTS
(define (divides? a b)
  (cond [(equal? a 0) (cond [(equal? b 0) #t]
                            [else #f]
                            )]
        [else (equal? (remainder b a) 0)]
   )
  )
...
(define (wj-solvable? cA cB g)
  (cond [(or (>= cA g) (>= cB g)) (cond [(divides? (gcd cA cB) g) #t]
                                        [else #f]
                                    )]
        [else #f]
   )
...
(define (waterjug cA cB g)
  (cond [(wj-solvable? cA cB g) (waterjug_algorithm cA cB g 0 0)]
        [else "Not solvable!"]
   )
  )

(define (waterjug_algorithm cA cB g amount_A amount_B)
  (cond
    ;if the amount in jug B is the same as the amount needed, return the final value. Step 1.
    [(equal? amount_B g) (list (list amount_A amount_B))]
        [else (cond
                ;if jug B is full, return the amount in jug A and B and remove all content of jug B. Step 2.
                [(equal? amount_B cB) (append (list(list amount_A amount_B)) (waterjug_algorithm cA cB g amount_A 0))]
                    [else (cond
                            ;if jug A is empty, return the amount in jug A and B and recursively run the algorithm again by filling up jug A. Step 3.
                            [(equal? amount_A 0) (append (list(list amount_A amount_B)) (waterjug_algorithm cA cB g cA amount_B))]
                                [else (cond
                                        ;pour from jug A into jug B is full. Step 4.
                                        [(> (+ amount_A amount_B) cB) (append (list(list amount_A amount_B)) (waterjug_algorithm cA cB g (remainder (+ amount_A amount_B) cB) cB))]
                                            [else (cond
                                                    ;fill up jug B. Step 5.
                                                    [(<= (+ amount_A amount_B) cB) (append (list(list amount_A amount_B)) (waterjug_algorithm cA cB g 0 (+ amount_A amount_B)))]
                                                        [else #f]
                                                   )]
                                       )]
                           )]
